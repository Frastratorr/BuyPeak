...
export default function AdminPage() {
  ...
  const handleSaveProduct = async () => {
    if (uploadingImage) {
        showNotification("Подождите загрузки фото...", "warning");
        return;
    }
  ...
  const handleToggleAdmin = async (targetUser) => {
    if (targetUser.email === "admin@gmail.com") return showNotification("Нельзя изменить роль главного админа", "warning");
  ...
  const handleToggleBlock = async (targetUser) => {
    if (targetUser.email === "admin@gmail.com") return showNotification("Нельзя заблокировать главного админа", "warning");
    if (targetUser.id === user.id) return showNotification("Нельзя заблокировать самого себя", "warning");
                ...
                {products.map((p, index) => (
                    ...
                    >
                        ...
                        <Stack direction="row" spacing={1} mt={2}>
                            ...
                            <IconButton color="error" onClick={() => handleDeleteProduct(p.id)} sx={{ bgcolor: '#ffebee' }}><DeleteIcon /></IconButton>
                        </Stack>
                    </Paper>
                ))}
            </Box>
